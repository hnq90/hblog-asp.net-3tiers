...
namespace Intelligencia.UrlRewriter.Utilities
{
    ...
    internal static class TypeHelper
    {
        ...
        {
            string[] components = fullTypeName.Split(new[] {','}, 2);
            if (components.Length != 2)
            {
                throw new ArgumentOutOfRangeException("fullTypeName", fullTypeName,
                                                      MessageProvider.FormatString(
                                                          Message.FullTypeNameRequiresAssemblyName));
            }
        ...
        public static object Activate(string assemblyName, string typeName, object[] args)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("typeName", typeName,
                                                      MessageProvider.FormatString(Message.TypeNameRequired));
            }

            return AppDomain.CurrentDomain.CreateInstanceAndUnwrap(assemblyName, typeName, false, 0, null, args, null,
                                                                   null, null);
        }
    }
}